...
本專案是一個 **ASUS 全球伺服器市場策略分析平台**，為 ASUS 伺服器業務團隊提供可搜尋、可篩選、可匯出的企業情報儀表板。涵蓋全球 **421 家企業**（NA 100、APAC 105、EMEA 116、China 100），包含採購量、供應商、難易度、策略分析、決策者聯絡資訊等 21 個欄位。

### 核心功能

| 功能 | 說明 |
|------|------|
| 雙語支援 (i18n) | 支援繁體中文與英文切換，包含 UI 介面與企業資料內容 |
| 區域切換 | NA（北美）、APAC（亞太）、EMEA（歐洲中東非洲）、China（中國大陸） |
| 搜尋 | 即時全文搜尋，涵蓋企業名稱、產業、供應商、ASUS 型號、決策者等 13 個欄位 |
...
| 難易度詳情 | 點擊難易度條可開啟 Modal，檢視 6 大維度的詳細評分與說明 |
...
| 路由 | Wouter | 3.x |
| 狀態管理 | Context API + LocalStorage | 用於主題與語言設定 |
...
│       ├── data/               ← ★ 靜態 JSON 資料（8 份檔案，中英雙語）
│       │   ├── companies.json & companies_en.json             ← NA 北美 100 家
│       │   ├── apac_companies.json & apac_companies_en.json   ← APAC 亞太 105 家
│       │   ├── emea_companies.json & emea_companies_en.json   ← EMEA 歐洲中東非洲 116 家
│       │   └── china_companies.json & china_companies_en.json ← China 中國大陸 100 家
│       ├── lib/
│       │   ├── regions.ts      ← ★ Company 型別定義 + 區域設定
│       │   ├── i18n.ts         ← ★ 語系翻譯字典檔 (150+ 詞條)
│       │   └── utils.ts        ← 工具函式（cn 等）
│       ├── pages/
│       │   ├── Home.tsx        ← ★ 主頁面（表格、篩選、排序、匯出）
│       │   ├── CompanyDetail.tsx ← 企業詳情頁
│       │   ├── DataStatus.tsx  ← 內部資料連線狀態頁面
│       │   └── NotFound.tsx    ← 404 頁面
│       ├── components/
│       │   ├── RegionMapBg.tsx ← 區域專屬 SVG 地圖背景
│       │   ├── DifficultyModal.tsx ← 難易度詳情彈出視窗
│       │   ├── StructuredText.tsx  ← 策略文字結構化渲染元件
│       │   └── ui/             ← shadcn/ui 元件（勿手動修改）
│       ├── contexts/
│       │   ├── ThemeContext.tsx ← 主題切換
│       │   └── LanguageContext.tsx ← 語系切換與狀態管理
│       └── hooks/              ← 自訂 hooks
...
所有企業資料以 **靜態 JSON 檔案** 存放在 `client/src/data/` 目錄下，分為中英雙語版本。Vite 在建置時會將其打包進 JavaScript bundle。
...
| `industry` | string | 選填 | 產業分類標籤（必須使用簡稱，如：半導體、企業軟體） |
...
1. 同時修改中文與英文的 `client/src/data/*.json` 檔案
...
### 區域專屬視覺 (Banner Design)

首頁頂部的 Banner 根據不同區域有專屬的視覺設計，由 `RegionMapBg.tsx` 元件渲染：
- **NA (北美)**：深藍色漸層 + 北美地圖
- **APAC (亞太)**：翡翠綠漸層 + 亞太地圖
- **EMEA (歐洲中東非洲)**：琥珀色漸層 + EMEA地圖
- **China (中國大陸)**：紅色漸層 + 中國地圖
*地圖背景包含輻射光暈、網格線、城市節點與連接線，營造科技感。*
...
| `/internal/data-status` | DataStatus | 內部資料連線狀態檢測頁面 |
| `/404` | NotFound | 404 頁面 |

路由使用 **Wouter**（輕量級路由庫），不是 React Router。
...
1. **雙語同步更新**：新增或修改資料時，必須同時更新中文與英文的 JSON 檔案。新增 UI 文字時，必須在 `i18n.ts` 中新增對應的詞條。
2. **不要修改 `client/src/components/ui/` 目錄**：這些是 shadcn/ui 自動產生的元件，如需客製化，應在使用處覆寫 props 或 className。
3. **色彩必須使用 CSS 變數**：不要硬編碼色彩值，使用 `bg-primary`、`text-foreground` 等語義化類別。
4. **JSON 資料修改後必須驗證**：每個企業必須包含所有必填欄位，`difficulty` 必須在 1-10 之間，`challenges` 和 `entryPoint` 必須包含 `【】` 結構化標題。
5. **匯出功能必須同步更新**：如果新增或修改 Company 欄位，必須同時更新 `exportToCSV()` 和 `exportToExcel()` 函式中的 headers 和 rows 映射。
6. **Vercel 部署使用 `build:client`**：絕對不要讓 Vercel 執行預設的 `build` 指令，否則會打包 server 導致部署失敗。
...
`hyperscaler`, `neocloud`, `financial`, `healthcare`, `defense`, `telecom`, `automotive`, `retail`, `energy`, `enterprise_general`。

### 結構化格式

- **Challenges**: `【{archetype前綴}】【技術門檻】...【供應鏈壁壘】...【商務挑戰】...【產業特性】...`
- **EntryPoint**: `【{archetype前綴}】【短期（6-12個月）】...【中期（1-2年）】...【長期（2-3年）】...【關鍵行動】...`

前端會透過 `StructuredText` 元件自動解析這些 `【】` 標籤並進行高亮渲染。
