...
> **專案目標**：這是一個「ASUS 全球伺服器市場拓展策略」的內部儀表板（Dashboard），主要用來展示全球 421 家頂尖企業的伺服器採購策略、難易度分析，以及各項產業的數據總覽，協助 ASUS 伺服器業務團隊快速查找客戶資訊與切入策略。

本文件為 **AI 專屬交接文件與開發指南**，確保任何接手的 AI 助手或開發者都能快速掌握專案的核心邏輯與開發規範。若需要更詳細的欄位定義與開發細節，請參閱 [`AI_INSTRUCTIONS.md`](./AI_INSTRUCTIONS.md)。
...
| **多語系 (i18n)** | Context API + LocalStorage | 支援繁體中文與英文雙語切換 |
| **部署** | Vercel | 透過 `vercel.json` 設定，使用 `build:client` 僅打包前端 |
| **資料** | 靜態 JSON | 4 個地區 × 2 種語言（中/英），共 8 份檔案，打包進前端 bundle |
...
資料存放在 `client/src/data/` 目錄下，分為四個區域，**共 421 筆資料**。為了支援雙語，每個區域都有對應的 `_en` 英文版本：

| 地區 | 中文資料檔 | 英文資料檔 | 筆數 | 總採購量 |
|------|------------|------------|------|----------|
| 北美 (NA) | `companies.json` | `companies_en.json` | 100 筆 | ~6.5M 台/年 |
| 亞太 (APAC) | `apac_companies.json` | `apac_companies_en.json` | 105 筆 | ~4.2M 台/年 |
| 歐洲中東非洲 (EMEA) | `emea_companies.json` | `emea_companies_en.json` | 116 筆 | ~2.8M 台/年 |
| 中國大陸 (China) | `china_companies.json` | `china_companies_en.json` | 100 筆 | ~5.0M 台/年 |

### 資料更新流程
未來新增或修改資料時，**請務必同時更新中文與英文的靜態 JSON 檔案**。
若新增了資料欄位，必須同步更新以下四個地方：
1. `client/src/lib/regions.ts` 中的 `Company` 型別定義
2. `client/src/data/*.json` 中的資料內容（含中英文）
...
- **區域專屬視覺**：每個區域的 Banner 都有專屬的漸層色彩與精緻的 SVG 地圖背景（含輻射光暈、網格線與城市節點）。
...
> 註：前端會透過 `StructuredText` 元件自動解析 `【】` 標籤並進行高亮渲染。
...
| 日期 | 事項 |
|------|------|
| 2026-04-23 | 由 Claude AI（Anthropic）正式接手專案維護，負責後續功能開發、資料更新與 bug 修復。 |
